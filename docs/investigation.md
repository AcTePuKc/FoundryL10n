...
- **Pipeline trace + unification callout (2026-01-17):**
  - `extract_response_text` is only called inside `LLMService` (`translate_segment` and `repair_placeholders`), which makes it the single response parsing adapter for the app.【F:src/services/llm_service.py†L152-L244】
  - **CLI path:** `src/main.py` (`translate_file`/`translate_text`) → `run_batch_translation` → `TranslationEngine` → `LLMService.translate_segment`/`repair_placeholders`.【F:src/main.py†L123-L260】【F:src/services/translation_service.py†L1-L34】【F:src/core/engine.py†L27-L424】
  - **GUI path:** `TranslationWorker`/`JSONLPipelineWorker` → `TranslationEngine` → `LLMService.translate_segment`/`repair_placeholders` (JSONL uses `process_chunk` but still runs through the same engine/LLM adapter).【F:src/ui/worker.py†L9-L183】【F:src/core/engine.py†L27-L424】
  - **Finding:** no CLI/JSONL/GUI path bypasses `LLMService` or uses a different response parser; all routes converge on `extract_response_text` via `LLMService`, so no refactor was required. The unification approach is to keep `extract_response_text` as the single adapter and avoid introducing parallel parsing paths.
- **Stub set (expand investigation, no execution yet):**
  ...
  - **2026-01-17 — Dependency version audit:** capture current `ollama` client version from the lockfile and confirm if it corresponds to the `GenerateResponse` change. **Write-back required:** note version(s) and any upstream changelog pointers here. [ ] TODO
  - **2026-01-17 — Dependency version audit (write-back):** `pyproject.toml` declares `ollama>=0.6.1` and `uv.lock` resolves `ollama==0.6.1` (requirements.txt is unpinned). Upstream reference: the v0.6.1 release notes only mention logprobs support and do not call out `GenerateResponse`; no explicit changelog entry was located for that type in the published GitHub releases (https://github.com/ollama/ollama-python/releases/tag/v0.6.1).
