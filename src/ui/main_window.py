...
class FoundryGUI(QMainWindow):
    ...
    def update_row_visuals(self, row_idx: int):
        ...
        has_remote_change = self._remote_change_ready and bool(
            getattr(seg, "remote_changed", False)
        )
        if self._remote_change_ready and not has_remote_change:
            has_remote_change = self._segment_change_key(seg) in self._remote_change_map
